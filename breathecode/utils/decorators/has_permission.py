...
HasPermissionCallback = Callable[
    [PermissionContextType, tuple, dict], tuple[PermissionContextType, tuple, dict, Optional[timedelta]]
]
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if validate_permission(request.user, permission, consumer):
                    ...
                    if (
                        consumer
                        and context["will_consume"]
                        and context["time_of_life"]
                        and (consumable := context["consumables"].first())
                    ):
                ...
                if format == "html":
                    ...
                    if permission == "join_mentorship":
                        subscription = Subscription.objects.filter(
                            user=request.user, selected_mentorship_service_set__mentorship_services__slug=service
                        ).first()
                        ...
                        subscription = Subscription.objects.filter(
                            user=request.user, selected_event_type_set__event_types__slug=service
                        ).first()
                        if subscription is not None:
