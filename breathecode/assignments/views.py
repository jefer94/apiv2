...
class GenericTaskView(APIView):
    ...
    def get(self, items: QuerySet[Task], academy_id=None):
        handler = self.extensions(self.request)
        cache = handler.cache.get()
        if cache is not None:
            return Response(cache, status=status.HTTP_200_OK)

        has_capability = bool(academy_id)
        ...
        if has_capability and (edu_status := self.request.GET.get('edu_status')):
            items = items.filter(user__cohortuser__educational_status__in=edu_status.split(','))

        # tasks from users that belong to these cohort
        if has_capability and (teacher := self.request.GET.get('teacher', None)):
    ...
    # this keep separated, the developer should decides if their view can get a task by id
...
class GenericFinalProjectView(APIView):
    ...


class AcademyFinalProjectView(GenericFinalProjectView):
    """
    List all snippets, or create a new snippet.
    """
    @capable_of('read_assignment')
...
class AcademyTaskView(GenericTaskView):
    ...
    """
    @capable_of('read_assignment')
    def get(self, _, task_id=None, user_id=None, academy_id=None):

        if task_id:
            return self.get_id(task_id, user_id)
        ...
        items = Task.objects.filter(**lookup)

        return super().get(items, academy_id=academy_id)

    @capable_of('crud_assignment')
    def put(self, request, task_id=None, academy_id=None):
        def update(_req, data, _id=None, only_validate=True):
            if _id is None:
                raise ValidationException('Missing task id to update', slug='missing=task-id')

            item = Task.objects.filter(Q(cohort__academy__id=academy_id) | Q(cohort__isnull=True),
                                       id=_id).first()
            ...
            if serializer.is_valid() and not only_validate:
                serializer.save()
    ...
    #FIXME if the staff remove a task, this should be considered as a task banned
...
class TaskMeView(GenericTaskView):
    ...
    def get(self, request, task_id=None):
        if task_id:
            return self.get_id(task_id, request.user.id)

        items = Task.objects.filter(user__id=request.user.id)
